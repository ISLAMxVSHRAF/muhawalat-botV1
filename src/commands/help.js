    ...
    return [
        ...
        new EmbedBuilder()
            ...
            .addFields(
                ...
                {
                    name: '📱 مساحة العضو (الداشبورد)',
                    value: [
                        'في مساحة كل عضو تظهر أزرار العادات والأهداف والإحصائيات.',
                        '**📝 تدوين** — يفتح نافذة لكتابة أفكارك وتدوينها **بسرية** في مساحتك الخاصة.',
                        '**🗂️ سجل التدوين** — يعرض تدويناتك السابقة مرتبة حسب التاريخ (رسالة خاصة بك فقط).',
                    ].join('\n'),
                    inline: false
                },
            )
        ...
        new EmbedBuilder()
            ...
            .addFields(
                ...
                },
                {
                    name: '📅 الشهر المخصص',
                    value: [
                        '`/start_month [duration:]` — يبدأ شهر مخصص جديد ويُعيد ضبط المتابعات (المدة الافتراضية 30 يوماً).',
                        '`/end_month` — يغلق الشهر المخصص الحالي يدوياً.',
                    ].join('\n'),
                    inline: false
                },
                {
                    name: '📌 المهام',
                    value: [
                        '`/task_create type: [image:]` — يفتح **Modal** لإدخال العنوان والوصف، ثم ينشر المهمة في المنتدى كنص عادي (+ صورة إن وُجدت). أسبوعية تقفل بعد 48 ساعة | شهرية بعد 5 أيام. العضو يسجل بكتابة رسالة فوق 10 كلمات (حد أقصى مرتين).',
                        '`/task_list` — يعرض المهام النشطة وموعد قفلها',
                        '`/sync_tasks thread_id:` — أدخل **معرف الثريد** فقط؛ تفتح نافذة بسيطة لاختيار النوع (أسبوعي/شهري) والرقم/الترتيب، ثم تُحفظ في الداتابيز ويتم مزامنة الإتمام من الرسائل.',
                    ].join('\n'),
                    inline: false
                },
            )
            ...
            .addFields(
                {
                    name: '🏆 التحديات',
                    value: [
                        '`/challenge_create [image:]` — يفتح **Modal** لإدخال العنوان والمحتوى، ثم ينشر البوست في قناة التحديات كنص عادي (+ صورة إن وُجدت). استخدم `/sync_challenge` لربط الثريد بالمدة والنقاط.',
                        '`/sync_challenge thread_id:` — أدخل **معرف ثريد التحدي** فقط؛ تفتح نافذة بسيطة (المدة بالأيام، إجمالي النقاط/وقت التحدي) ويُربط التحدي في الداتابيز.',
                        'العضو يسجل بكتابة ✅ + الوقت — مثال: `تم ✅ - 30 دقيقة`. التحدي يقفل ويعلن الفائزين تلقائياً عند انتهاء وقته.',
